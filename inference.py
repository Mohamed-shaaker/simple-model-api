...
# Pre-built transform pipeline constructed once at module import, not per-request.
# Pure function chain with no learnable parameters — safe to share across threads.
...
def preprocess_image(image_bytes: bytes) -> PreprocessResult:
    ...
    Error handling strategy (FR-3.2):
        ...
        to a clean PreprocessResult. The worker process is never killed.
    ...
    # RGBA -> drops alpha; L (grayscale) -> replicates to 3 channels;
    # P (palette) -> expands to RGB; CMYK -> converts color space.
    try:
        if image.mode != "RGB":
    ...
    # This is the PyTorch-heavy stage where OOM and dimension errors can surface.
    try:
        tensor: torch.Tensor = _preprocess_pipeline(image)
    except RuntimeError as exc:
        # FR-3.2: Catch PyTorch OOM and dimension mismatches, return clean error.
    ...
    # Stage 4: Add batch dimension (3, 224, 224) -> (1, 3, 224, 224)
    try:
